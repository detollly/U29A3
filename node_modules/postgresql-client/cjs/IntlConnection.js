...
const PgSocket_js_1 = require("./protocol/PgSocket.js");
...
class IntlConnection extends SafeEventEmitter_js_1.SafeEventEmitter {
    constructor(config) {
        super();
        this._refCount = 0;
        ...
        this.socket = new PgSocket_js_1.PgSocket(this._config);
    ...
    async connect() {
        if (this.socket.state === definitions_js_1.ConnectionState.READY)
            return;
        debug("connecting");
    ...
    async close() {
        if (this.state === definitions_js_1.ConnectionState.CLOSED)
            return;
        debug("[%s] closing", this.processID);
        this.statementQueue.clearQueue();
        return new Promise((resolve) => {
            if (this.socket.state === definitions_js_1.ConnectionState.CLOSED)
                return;
    ...
    assertConnected() {
        if (this.state === definitions_js_1.ConnectionState.CLOSING)
    ...
    async _execute(sql, options, cb) {
        ...
        try {
            ...
            let fields;
            const typeMap = opts.typeMap || DataTypeMap_js_1.GlobalTypeMap;
            return await this.socket.capture(async (code, msg, done) => {
                switch (code) {
                    ...
                    case protocol_js_1.Protocol.BackendMessageCode.RowDescription:
                        fields = msg.fields;
                        parsers = (0, common_js_1.getParsers)(typeMap, fields);
                        current.fields = (0, common_js_1.wrapRowDescription)(typeMap, fields, DataFormat.text);
                        current.rows = [];
                        break;
                    case protocol_js_1.Protocol.BackendMessageCode.DataRow:
                        let row = msg.columns.map((x) => x.toString("utf8"));
                        (0, common_js_1.parseRow)(parsers, row, opts);
                        if (opts.objectRows && current.fields)
                            row = (0, common_js_1.convertRowToObject)(current.fields, row);
                        if (cb)
                            cb("row", row);
