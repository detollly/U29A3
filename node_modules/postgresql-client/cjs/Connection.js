...
const BindParam_js_1 = require("./BindParam.js");
const DataTypeMap_js_1 = require("./DataTypeMap.js");
const definitions_js_1 = require("./definitions.js");
const IntlConnection_js_1 = require("./IntlConnection.js");
const PreparedStatement_js_1 = require("./PreparedStatement.js");
const SafeEventEmitter_js_1 = require("./SafeEventEmitter.js");
const debug = (0, debug_1.default)("pgc:intlcon");
class Connection extends SafeEventEmitter_js_1.SafeEventEmitter {
    constructor(arg0, arg1) {
        super();
        this._notificationListeners = new SafeEventEmitter_js_1.SafeEventEmitter();
        this._closing = false;
        if (arg0 && typeof arg0.acquire === "function") {
            if (!(arg1 instanceof IntlConnection_js_1.IntlConnection))
                throw new TypeError("Invalid argument");
            this._pool = arg0;
            this._intlCon = arg1;
        }
        else
            this._intlCon = new IntlConnection_js_1.IntlConnection(arg0);
    ...
    async connect() {
        ...
        if (this.state === definitions_js_1.ConnectionState.READY)
            this._closing = false;
    }
    /**
    ...
    async close(terminateWait) {
        ...
        if (this.state === definitions_js_1.ConnectionState.CLOSED || this._closing)
            return;
    ...
    async query(sql, options) {
        ...
        const typeMap = options?.typeMap || DataTypeMap_js_1.GlobalTypeMap;
        const paramTypes = options?.params?.map((prm) => prm instanceof BindParam_js_1.BindParam ? prm.oid : typeMap.determine(prm) || definitions_js_1.DataTypeOIDs.varchar);
        const statement = await this.prepare(sql, { paramTypes, typeMap }).catch((e) => {
            throw this._handleError(e, sql);
        });
        try {
            const params = options?.params?.map((prm) => (prm instanceof BindParam_js_1.BindParam ? prm.value : prm));
    ...
    async prepare(sql, options) {
        ...
        return await PreparedStatement_js_1.PreparedStatement.prepare(this, sql, options);
    }
    /**
