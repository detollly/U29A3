...
const DatabaseError_js_1 = require("./DatabaseError.js");
...
class PgSocket extends SafeEventEmitter_js_1.SafeEventEmitter {
    constructor(options) {
        super();
        this.options = options;
        this._state = definitions_js_1.ConnectionState.CLOSED;
        this._backend = new Backend_js_1.Backend();
        this._frontend = new Frontend_js_1.Frontend();
    ...
    connect() {
        if (this._socket)
            return;
        this._state = definitions_js_1.ConnectionState.CONNECTING;
    ...
    close() {
        ...
        if (this._state === definitions_js_1.ConnectionState.CLOSING)
            return;
        const socket = this._socket;
        this._state = definitions_js_1.ConnectionState.CLOSING;
        this._removeListeners();
    ...
    _handleConnect() {
        ...
        if (!socket)
            return;
        this._state = definitions_js_1.ConnectionState.AUTHORIZING;
        this._reset();
    ...
    _handleError(err) {
        if (this._state !== definitions_js_1.ConnectionState.READY) {
            this._socket?.end();
        }
        this.emit("error", err);
    }
            ...
            try {
                switch (code) {
                    ...
                    case protocol_js_1.Protocol.BackendMessageCode.ErrorResponse:
                        this.emit("error", new DatabaseError_js_1.DatabaseError(payload));
                        break;
                    ...
                    case protocol_js_1.Protocol.BackendMessageCode.ReadyForQuery:
                        if (this._state !== definitions_js_1.ConnectionState.READY) {
                            this._state = definitions_js_1.ConnectionState.READY;
                            this.emit("ready");
                        }
                        else
