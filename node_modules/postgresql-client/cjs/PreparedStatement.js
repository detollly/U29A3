...
let portalCounter = 0;
class PreparedStatement extends SafeEventEmitter_js_1.SafeEventEmitter {
    ...
    static async prepare(connection, sql, options) {
        const intoCon = (0, common_js_1.getIntlConnection)(connection);
    ...
    async execute(options = {}) {
        ...
        const intlCon = (0, common_js_1.getIntlConnection)(this.connection);
    ...
    async close() {
        ...
        if (this._refCount > 0)
            return;
        const intoCon = (0, common_js_1.getIntlConnection)(this.connection);
    ...
    async _execute(options = {}) {
        ...
        try {
            ...
            if (fields) {
                ...
                resultFields = (0, common_js_1.wrapRowDescription)(typeMap, fields, options.columnFormat || definitions_js_1.DEFAULT_COLUMN_FORMAT);
            ...
            if (resultFields && parsers && executeResult.rows) {
                ...
                for (let i = 0; i < l; i++) {
                    row = rows[i];
                    (0, common_js_1.parseRow)(parsers, row, options);
                    if (options.objectRows) {
                        rows[i] = (0, common_js_1.convertRowToObject)(resultFields, row);
                    }
                }
            }
