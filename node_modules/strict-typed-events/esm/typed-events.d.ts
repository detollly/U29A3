...
export declare type Listener<TArgs extends any[] = any[]> = (...args: TArgs) => any;
...
export declare type TypedEventEmitter<TEmitter, TEventRecord, TEmitRecord = TEventRecord> = _TypedEventEmitter<TEmitter, TEventRecord, TEmitRecord>;
declare type _TypedEventEmitter<TEmitter, TEventRecord, TEmitRecord, TEventEmitterOverride = _EventEmitterOverride<TEmitter, TEventRecord, TEmitRecord>, TKeysToOverride extends keyof TEventEmitterOverride = Extract<keyof TEventEmitterOverride, keyof TEmitter>> = Pick<TEmitter, Exclude<keyof TEmitter, TKeysToOverride>> & Pick<TEventEmitterOverride, TKeysToOverride>;
declare type _ListenerParameters<T> = [T] extends [(...args: infer U) => any] ? U : [T] extends [void] ? [] : [T];
declare const _assignmentCompatibilityHack: unique symbol;
declare type _EventEmitterOverride<TEmitter, TEventRecord, TEmitRecord = TEventRecord, TEventRecordKeys extends keyof TEventRecord = Extract<keyof TEventRecord, FunctionKeys<TEventRecord>>, TEmitRecordKeys extends keyof TEmitRecord = Extract<keyof TEmitRecord, FunctionKeys<TEmitRecord>>> = {
